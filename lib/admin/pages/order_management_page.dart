...
class _OrderManagementPageState extends ConsumerState<OrderManagementPage>
  ...
  Widget build(BuildContext context) {
    ...
    // Masuk = tungguKonfirmasi yang belum diaccept admin
    ...
    // Diproses = sudah diterima admin, status diproses
    final processing = orders
        .where((o) =>
            o.acceptedByAdmin && o.status == OrderStatusModel.diproses)
        .toList();
    ...
    final done =
        orders.where((o) => o.status == OrderStatusModel.selesai).toList();
    final cancelled =
        orders.where((o) => o.status == OrderStatusModel.dibatalkan).toList();
            ...
            Expanded(
              child: TabBarView(
                ...
                children: [
                  ...
                  _OrderList(orders: processing, mode: _CardMode.processing),
...
enum _CardMode { incoming, processing, delivering, done }
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                      ...
                      child: GestureDetector(
                        onTap: () async {
                          ...
                          await notifier.updateOrderStatus(
                              order.id, OrderStatusModel.dibatalkan);
                ...
                // Tab Diproses: Konfirmasi Dijalan (assign driver otomatis)
                if (mode == _CardMode.processing) ...[
                  const SizedBox(height: 16),
                  SizedBox(
                    width: double.infinity,
                    child: GestureDetector(
                      onTap: () async {
                        final confirm = await _confirmDialog(
                          ...
                          message:
                              'Pesanan akan ditandai sedang dalam perjalanan ke user. Driver akan di-assign otomatis.',
                        ...
                        await notifier.updateOrderStatusWithDriver(
                            order.id, OrderStatusModel.diantar, driver);

                        notifNotifier.addNotification(
                          title: 'Pesanan Dalam Perjalanan 🛵',
                          message:
                              'Pesanan #${order.id.substring(8)} sedang dalam perjalanan menuju lokasimu. Driver: ${driver.name} (${driver.vehicleNumber}).',
                          orderId: order.id,
                        );
                      },
                      child: Container(
                    ...
                    Container(
                      ...
                      child: Row(
                        children: [
                          ...
                          Icon(Icons.delivery_dining_rounded,
                              color: Colors.indigo.withOpacity(0.6), size: 20),
                        ],
                      ),
                    ),
